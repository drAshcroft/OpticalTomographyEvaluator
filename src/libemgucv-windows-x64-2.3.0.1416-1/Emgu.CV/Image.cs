...
namespace Emgu.CV
{
        ...
        where TDepth : new()
    {
        ...
        protected void LoadFileUsingBitmap(FileInfo file)
        {
        ...
        /// </summary>
        /// <param name="file"></param>
        protected void LoadImageUsingOpenCV(FileInfo file)
        {
        ...
        /// <summary>
        /// performs forward  2D floating-point array
        /// </summary>
        /// <returns>The result of DFT</returns>
        [ExposableMethod(Exposable = true, Category = "Discrete Transforms")]
        public Image<Gray, Single> FFT2(Image<TColor, Single> image)
        {
            ...
            Image<Gray, float> im1 = new Image<Gray, float>(outReal.Width, outReal.Height);
            outReal.CopyTo(im1);

            Image<Gray, float> im2 = new Image<Gray, float>(outReal.Width, outReal.Height);
            outIm.CopyTo(im2);

            // return new Image<Gray, float>(new Image<Gray, float>[] { im1, im2 });
            return im1;
        }
        ...
        /// <returns></returns>
        [ExposableMethod(Exposable = true, Category = "Discrete Transforms")]
        public Image<Gray, Single> ConvolveWith(Image<TColor, Single> image2)
        {
            //Image<Gray, float> image = new Image<Gray, float>(open.FileName);
            IntPtr complexImage1 = CvInvoke.cvCreateImage(Size, Emgu.CV.CvEnum.IPL_DEPTH.IPL_DEPTH_32F, 2);

            CvInvoke.cvSetZero(complexImage1);  // Initialize all elements to Zero
            CvInvoke.cvSetImageCOI(complexImage1, 1);
            CvInvoke.cvCopy(Ptr, complexImage1, IntPtr.Zero);
            CvInvoke.cvSetImageCOI(complexImage1, 0);

            Matrix<float> dft = new Matrix<float>(Rows, Cols, 2);
            CvInvoke.cvDFT(complexImage1, dft, Emgu.CV.CvEnum.CV_DXT.CV_DXT_FORWARD, 0);


            IntPtr complexImage2 = CvInvoke.cvCreateImage(image2.Size, Emgu.CV.CvEnum.IPL_DEPTH.IPL_DEPTH_32F, 2);

            CvInvoke.cvSetZero(complexImage2);  // Initialize all elements to Zero
            CvInvoke.cvSetImageCOI(complexImage2, 1);
            CvInvoke.cvCopy(image2.Ptr, complexImage2, IntPtr.Zero);
            CvInvoke.cvSetImageCOI(complexImage2, 0);

            Matrix<float> dft2 = new Matrix<float>(Rows, Cols, 2);
            CvInvoke.cvDFT(complexImage2, dft2, Emgu.CV.CvEnum.CV_DXT.CV_DXT_FORWARD, 0);


            //The Real part of the Fourier Transform
            Matrix<float> A2 = new Matrix<float>(Size);
            //The imaginary part of the Fourier Transform
            Matrix<float> B2 = new Matrix<float>(Size);
            CvInvoke.cvSplit(dft, A2, B2, IntPtr.Zero, IntPtr.Zero);

            float  w = A2.Data[0, 0] + B2.Data[0, 0];
            ////The Real part of the Fourier Transform
            //Matrix<float> C = new Matrix<float>(Size);
            ////The imaginary part of the Fourier Transform
            //Matrix<float> D = new Matrix<float>(Size);
            //CvInvoke.cvSplit(dft, C, D, IntPtr.Zero, IntPtr.Zero);

            Image<Gray, float> im1 = new Image<Gray, float>(Width, Height);
            float A, B, C, D;
            for (int x = 0; x < dft.Data.GetLength(1); x += 2)
            {
                for (int y = 0; y < dft.Data.GetLength(0); y++)
                {
                    A = dft.Data[y, x];
                    B = dft.Data[y, x + 1];
                    ...
                    C = dft2.Data[y, x];
                    D = dft2.Data[y, x + 1];
                    dft.Data[y, x] = A * C - B * D;
                    dft.Data[y, x + 1] = (A * D + B * C);
                }
            }

            CvInvoke.cvDFT(dft, dft2, Emgu.CV.CvEnum.CV_DXT.CV_DXT_INV_SCALE , 0);

            Matrix<float> D2 = new Matrix<float>(Size);
            CvInvoke.cvSplit(dft2, im1, D2, IntPtr.Zero, IntPtr.Zero);
        ...
        /// <returns></returns>
        [ExposableMethod(Exposable = true, Category = "Discrete Transforms")]
        public Image<Gray, Single> CorrelateWith(Image<TColor, Single> image2)
        {
            //Image<Gray, float> image = new Image<Gray, float>(open.FileName);
            IntPtr complexImage1 = CvInvoke.cvCreateImage(Size, Emgu.CV.CvEnum.IPL_DEPTH.IPL_DEPTH_32F, 2);
        ...
        /// <summary>
        /// Returns a better bitmap for floats
        /// </summary>
            ...
            get
            {
                ...
                if (typeOfColor == typeof(Gray)) // if this is a gray scale image
                {
                    ...
                    else
                    {
                        ...
                        using (Image<Gray, float> temp = Convert<Gray, float>())
                        {
                            float[, ,] Data = temp.Data;

                            double iMax = double.MinValue;
                            ...
                            for (int y = 0; y < iHeight; y++)
                                ...
                                {
                                    v = (float)Data[y, x, 0];
                                    if (iMax < v) iMax = Data[y, x, 0];
                                    if (iMin > v && v != -100000 && v != 0) iMin = Data[y, x, 0];
                                }
                            ...
                            unsafe
                            {
                                for (int y = 0; y < iHeight; y++)
                                {
                                    ...
                                    for (int x = 0; x < iWidth; x++)
                                    {
                                        ...
                                        int g = (int)(255d * (Data[y, x, 0] - iMin) / iLength);
                                        if (g > 255) g = 255;
                                        if (g < 0) g = 0;
                                        byte g2 = (byte)g;
                                        bits[0] = g2;
                ...
                {
                    throw new Exception("Not yet Implemented for anything but gray");
                }
            }
        }
        ...
        /// <summary>
        /// Threshold data using OTSU method
        /// </summary>
        /// <param name="threshold"></param>
        /// <param name="maxValue"></param>
        /// <returns></returns>
        [ExposableMethod(Exposable = true, Category = "Threshold")]
        public Image<TColor, TDepth> ThresholdOTSU(TColor threshold, TColor maxValue)
        {
            Image<TColor, TDepth> result = CopyBlank();
            ThresholdBase(result, threshold, maxValue, CvEnum.THRESH.CV_THRESH_OTSU | CvEnum.THRESH.CV_THRESH_BINARY);
            return result;
        }

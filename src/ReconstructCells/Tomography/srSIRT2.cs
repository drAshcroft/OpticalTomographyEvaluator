...
{
    public class obdSIRTRecon2 : ReconTemplate
    {
        #region Properties
        ...
        public float[, ,] DoProjections()
        {
          ...
          //  var temp = Library[10].FFT2(Library[10]);
            //var temp = Library[10].DFT(Emgu.CV.CvEnum.CV_DXT.CV_DXT_SCALE);
            //int w = temp.Width;
          ...
          //   BatchDeconv(9);
            ...
            SetConstants(ArrayPadDepad(Library[1], DensityGrid.GetLength(0) + 1));
            ...
            for (int i = 0; i < Library.Count; i++)
                Library[i] = (ArrayPadDepad(Library[i], DensityGrid.GetLength(0) + 1));
        ...
        private Image<Gray, float> DeconvolveImage(Image<Gray, float> image, int nMacroIter, int nPSFIter, int nImageIter)
        {
            ...
            Image<Gray,float> temp;

            for (int i = 0; i < nMacroIter; i++)
            {
                temp = psfBounds.Copy();
                temp = temp.Mul(1 / temp.Norm);
                X = obd2(X, image, temp, new int[] { nPSFIter, nImageIter }, 1, false, psfBounds);
            }
            X = (ArrayPadDepad(X, image.Width));
            return X;
        }
        ...
        private int[] size(Image<Gray,float> x)
        {
            return new int[] { x.Height , x.Width };
        }
        ...
        #endregion
        private Image<Gray, float> obd2(Image<Gray, float> x, Image<Gray, float> y, Image<Gray, float> f, int[] maxiter, double srf, bool regularize, Image<Gray, float> PSFBounds)
        {
            ...
            if (x != null)
            {
                ...
                obd_updatePSF(ref f, x, y, maxiter[1], srf, regularize, PSFBounds);
                double fSum = f.Data .SumArray();
                f.Data .DivideInPlace((float)fSum);
            ...
            else
            {
                //if ((isempty(f) == 1))
                {
                    f = new Image<Gray,float> ((int)sf,(int)sf);
                    int sf2 = (int)Math.Ceiling(sf / 2);
                    f.Data [sf2, sf2, 0] = 1;   //a delta peak

                }

                var sx = sy[0] + sf - 1;
                //  var xData = pos(cnv2tp(f, y, srf));
                x = pos(ArrayPadDepad(y, (int)sx));
                return x;
            }

            //improve true image x with multiplicative updates
            obd_update(ref x, f, y, maxiter[1], srf);
            return x;
        }


        //%%%%%%%%%%%%%%%%
        private void obd_updatePSF(ref Image<Gray, float> f, Image<Gray, float> x, Image<Gray, float> y, int iters, double srf, bool regularize, Image<Gray, float> psfBounds)
        {
            ...
            if (regularize)
            {
                for (int i = 0; i < iters / 2; i++)
                {
                    for (int j = 0; j < 2; j++)
                    {
                        ...
                        nom.Data.AddToArray(tol);
                        denom.Data.AddToArray(tol);
                        factor.DivideToArray(nom.Data , denom.Data);
                        //  factor = reshape(factor, sf);
                        //  imagesc(factor);drawnow;
                        f.Data .MultiplyInPlace(factor);
                    }

                    for (int xx = 0; xx < psfBounds.Height; xx++)
                        for (int yy = 0; yy < psfBounds.Width; yy++)
                        {
                            if (f.Data[xx, yy, 0] > psfBounds.Data[xx, yy, 0])
                                f.Data[xx, yy, 0] = psfBounds.Data[xx, yy, 0];
                        }
                }
        ...
        //%%%%%%%%%%%%%%%%
        private void obd_update(ref  Image<Gray, float> x1, Image<Gray, float> f, Image<Gray, float> y, int iters, double srf)
        {
            //depending on the value of sf the roles of f and x can be swapped
            var sf = size(x1);
            var sy = size(y);    //for srf > 1, the low resolution y
            ...
            for (int i = 0; i < iters; i++)
            {
                ...
                //  factor = reshape(factor, sf);
                //  imagesc(factor);drawnow;
                x1.Data.MultiplyInPlace(factor);
            }

        }

        //%%%%%%%%%%%%%%%%%
        private Image<Gray, float> pos(Image<Gray, float> x)
        {
            x.Data.EnforcePositive();
            return x;
        }

        //%%%%%%%%%%%%%%%%%
        private Image<Gray, float> cnv2slice(Image<Gray, float> A, int[] i, int[] j)
        {
            ...
            for (int I = i[0]; I < i[1]; I++)
            {
                ...
                {
                    aN[x, y, 0] = A.Data[I, J, 0];
                    y++;
                }
                x++;
            }
            return new Image<Gray,float>(aN);
        }

        private Image<Gray, float> cnv2slice(Image<Gray, float> A, int x1,int x2, int y1, int y2)
        {
            float[, ,] aN = new float[x2 - x1, y2-y1, 1];
            int x = 0, y = 0;
            for (int I = x1; I < x2; I++)
            {
                y = 0;
        ...
        //%%%%%%%%%%%%%%%%%
        private /*y*/  Image<Gray, float> cnv2(Image<Gray, float> x, Image<Gray, float> f, int sy)
        {
            var sx = size(x);
            var sf = size(f);
            Image<Gray,float> y = null;
            if (sx[0] >= sf[0]) //x is larger or equal to f
            {
                //perform convolution in Fourier space
                // y = ifft2(fft2(x) .* fft2(f, sx(1), sx(2)));

                Image<Gray, float> t2 = new Image<Gray, float>(MathFFTHelps.FFT_cnv2(x.Data, f.Data));

                Image<Gray,float> temp = ArrayPadDepad(f,sx[0]);// new Image<Gray,float>(sx[0],sx[1]);
                y= x.ConvolveWith(temp);
                y.Data = y.Data.MakeFFTHumanReadable();

                int w = t2.Width;
             //   y = ArrayPadDepad(y, sy);
              //  y = new Image<Gray,float>( MathFFTHelps.FFT_cnv2(x.Data , f.Data ));
            ...
            if ((sy < size(y)[0]))
            {
                y = ArrayPadDepad(y, sy);   //downsample
        ...
        //%%%%%%%%%%%%%%%%%
        private /* f*/  Image<Gray, float> cnv2tp(Image<Gray, float> x, Image<Gray, float> y, double srf)
        {
            var sx = size(x);
            Image<Gray,float> y2 = y;
            if (srf > 1)
            {
                y2 = ArrayPadDepad(y, floor(srf * size(y)[0]));    //upsample
            }
            var sy = size(y)[0];

            int sf;
            Image<Gray,float> f = null;
            //perform the linear convolution in Fourier space
            if (sx[0] >= sy)
            {
                sf = sx[0] - sy + 1;
                Image<Gray,float> f2 =new Image<Gray,float>( MathFFTHelps.conv2(x.Data , y.Data , sf)); //ifft2(conj(fft2(x)).*fft2(cnv2pad(y, sf)));

                Image<Gray, float> temp = ArrayPadDepad(y,x.Width );// sf);// new Image<Gray,float>(sx[0],sx[1]);
                f = x.CorrelateWith(temp);

               // f = cnv2slice(f, 0,sf, 0,sf);
              // 

              
                f.Data = f.Data.MakeFFTHumanReadable();
                f = ArrayPadDepad(f, sf);
                int w = f2.Width;
            ...
            else
            {
                if (sx[0] <= sy)
                {
                    ...
                    Image<Gray, float> temp = ArrayPadDepad(y, sf);// new Image<Gray,float>(sx[0],sx[1]);
                    Image<Gray, float> temp2 = ArrayPadDepad(x, sf);
                    f = temp2.CorrelateWith(temp);
                    f.Data = f.Data.MakeFFTHumanReadable();
                    f = ArrayPadDepad(f, sf);
                   // f.Data = f.Data.MakeFFTHumanReadable();

                    var   f2 =new Image<Gray,float>( MathFFTHelps.CrossCorrelationFFT(y.Data , x.Data , sf));
        ...
        //%%%%%%%%%%%%
        private Image<Gray, float> ArrayPadDepad(Image<Gray, float> x, int sy)
        {
            Image<Gray, float> imOut = new Image<Gray, float>(sy, sy);
            float[, ,] arrayOutf =imOut.Data;
            int startI = (int)(Math.Floor((x.Height - sy) / 2d));
            ...
            if (startI >= 0)
            {
                for (int i = 0; i < arrayOutf.GetLength(0); i++)
                {
                    ...
                    {
                        arrayOutf[i, j, 0] = (float)(x.Data[i + startI, j + startI, 0]);
                    }
                }
            ...
            else
            {
                for (int i = 0; i < x.Height; i++)
                {
                    ...
                    {
                        arrayOutf[i - startI, j - startI, 0] = (float)(x.Data[i, j, 0]);
                    }
                }

...
{
    public class MRF_IRT : ReconTemplate
    {
        #region Properties
        ...
        {
            float  observed, testvalue, testStep, currentValue, bestValue, bestTest;
            float predicted,  d1, d2, d3, s1, s2;
            float weightMRF = (float)(.5 / 6 / noiseVar);
                        ...
                        {
                            {
                                ...
                                predicted = observed + ErrorGrid[sliceNumber, x, y];
                                ...
                                testStep = (s2 - s1) / 3;
                                for (testvalue = s1; testvalue < s2; testvalue += testStep)
                                {

                                    d1 = (testvalue - observed);
                                    d1 = d1 * d1 / noiseVar;
                                ...
                                testvalue = predicted * lambda + bestTest * weight;

                                if (testvalue > 0)
                                    DensityGrid[sliceNumber, x, y] = testvalue;
                                else
           ...
           // ParallelOptions po = new ParallelOptions();
            ...
            for (int j = 0; j < nIterations; j++)
            {
                ...
                if (j == 0)
                {
                    ...
                    float adjustmentC = Math.Abs(.05f * averageGrid / aveError);
                ...
                ImageProcessing.ImageFileLoader.Save_TIFF(Program.DataFolder + "\\CrossSections_X_mrf_" + j + ".tif", CrossSections[0]);

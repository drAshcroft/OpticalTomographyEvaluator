...
{
    public class StatisticalRecon : ReconTemplate
    {
        ...
        private void BatchAddAndZero(int sliceNumber)
        {
            ...
            double testStep = 2 * testRange / 10;
            float error, d, val;
            double testvalue, d1, d2, d3;
            double currentValue = 0, bestValue = 0, bestTest;
            for (int x = 1; x < DensityGrid.GetLength(1) - 1; x++)
                for (int y = 1; y < DensityGrid.GetLength(2) - 1; y++)
                {
                    if (MaxR > (x - halfI) * (x - halfI) + (y - halfJ) * (y - halfJ) + (sliceNumber - halfK) * (sliceNumber - halfK))
                    {
                        ...
                        bestTest = val;
                        for (testvalue = val - testRange; testvalue < val + testRange; testvalue += testStep)
                        {
                            d1 = (testvalue - d);
                            d1 = d1 * d1 / covar;

                            #region neighbor values
                            d3 = DensityGrid[sliceNumber, x + 1, y] - testvalue;
                            d3 = d3 * d3;
                            d2 = ((d3 > maxDif) ? maxDif : d3);

                            d3 = DensityGrid[sliceNumber, x - 1, y] - testvalue;
                            d3 = d3 * d3;
                            d2 = d2 + ((d3 > maxDif) ? maxDif : d3);

                            d3 = DensityGrid[sliceNumber, x, y + 1] - testvalue;
                            d3 = d3 * d3;
                            d2 = d2 + ((d3 > maxDif) ? maxDif : d3);
                            ...
                            d3 = DensityGrid[sliceNumber + 1, x, y] - testvalue;
                            d3 = d3 * d3;
                            d2 = d2 + ((d3 > maxDif) ? maxDif : d3);
                            ...
                            d2 = d2 + ((d3 > maxDif) ? maxDif : d3);
                            #endregion
                            currentValue = d1 + neighborWeight * d2/covar ;
        ...
        public float[, ,] DoProjections(float[, ,] densityGrid)
        {
            ...
            for (int i = 1; i < densityGrid.GetLength(0) - 1; i++)
            {
                covar += Math.Abs(densityGrid[i, (int)halfI,(int) halfI] - densityGrid[i,(int) halfI + 1,(int) halfI]);

            }

            covar = covar / densityGrid.GetLength(0);
            testRange = 10*7 * covar;
            maxDif = 5 * covar;
            ...
            for (int j = 0; j < nIterations; j++)
            {
                ...
                //    BatchAddAndZero(i);
                errorWeight = (float)(10.0 * batchSkip / (numberOfImages) / Math.Sqrt (j + 2));
                sum += s2 + s3;

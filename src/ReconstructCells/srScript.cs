...
namespace ReconstructCells
{
    public class superResScript
    {
        public static void RunReconScript(string registrationFolder)
        {
           ...
           //Background.RemoveCapillaryTV tv = new Background.RemoveCapillaryTV();
           // tv.SetInput(ll.GetOutput());
           // tv.RunNode();
            //OnDemandImageLibrary lib2 = new OnDemandImageLibrary(pass.Library.Count, true, @"c:\temp", false);
            //for (int i = 0; i < pass.Library.Count; i++)
            //    lib2[i] = pass.Library[i].Copy();
         ...
         //   roughReg.setBackground(theBackground);
           ...
           // return;
            ...
            pass.SavePassData(@"c:\temp\superRes");
          ...
          //  return;
           ...
           // ps.GetOutput().DensityGrid.ScrubReconVolume(ps.GetOutput().Library.Count);
            ...
            ImageProcessing.ImageFileLoader.SaveDensityData(@"c:\temp\superres\density.raw", ps.GetOutput().DensityGrid, ImageFileLoader.RawFileTypes.Float32);
            //#region TIK

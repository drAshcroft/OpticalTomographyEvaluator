...
namespace ZedGraph
{
	...
	#region Events
        ...
        public delegate int AddPointEditHandler(ZedGraphControl sender, GraphPane pane,
            CurveItem curve, int iPt, PointF mousePT);
        ...
        [Bindable(true), Category("Events"),
         Description("Subscribe to this event to respond to data point edit actions")]
        public event AddPointEditHandler AddPointEditEvent;
	...
	#region Mouse Events
		...
		protected void ZedGraphControl_MouseDown( object sender, MouseEventArgs e )
		{
				 ...
				 ( e.Button == EditButtons && Control.ModifierKeys == EditModifierKeys ) )
			{
                ...
                {
                    if (AddPointEditEvent != null)
                    {
                        double x, x2, y, y2;
                       pane.  ReverseTransform(mousePt, out x, out x2, out y, out y2);

                        int _newIndex = AddPointEditEvent(this, pane, _dragCurve, _dragIndex, new PointF((float)x,(float)y));
                        if (  _newIndex!=-1 )
                        {
                            _dragIndex = _newIndex;
                            _isEditing = true;
                            _dragPane = pane;
                            _dragStartPt = mousePt;
                            _dragStartPair = _dragCurve[_dragIndex];
